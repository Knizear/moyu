...
if (a.equals("insert")) {
    // 执行插入订单订单信息模块,表名：dingdandingdanxinxi
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().insert(request, "dingdandingdanxinxi", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    new CommDAO().commOper("UPDATE dingdandingdanxinxi SET xiaoji=(goumaishuliang*chanpinjiage) WHERE id='" + charuid + "'");
    ...
    // 执行更新订单订单信息模块,表名：dingdandingdanxinxi
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().update(request, "dingdandingdanxinxi", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法

    new CommDAO()
        .commOper(
            "UPDATE dingdandingdanxinxi SET xiaoji=(goumaishuliang*chanpinjiage) WHERE id='" + request.getParameter("id") + "'"
        );
    ...
    // 获取一行数据
    Map map = Query.make("dingdandingdanxinxi").find(Request.get("id"));

    new CommDAO().delete(request, "dingdandingdanxinxi"); // 执行删除命令
