...
if (a.equals("insert")) {
    ...
    String charuid = new CommDAO().insert(request, "dingdan", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    new CommDAO()
        .commOper(
            "INSERT INTO dingdandingdanxinxi(dingdanid,chanpinxinxiid,chanpinmingcheng,chanpinfenlei,chanpintupian,chanpinjiage,goumaishuliang,xiaoji)  SELECT '" +
            charuid +
            "',chanpinxinxiid,chanpinmingcheng,chanpinfenlei,chanpintupian,chanpinjiage,goumaishuliang,xiaoji  FROM gouwuche  WHERE goumairen='" +
    ...
    String charuid = new CommDAO().update(request, "dingdan", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    Map map = Query.make("dingdan").find(Request.get("id"));

    new CommDAO().delete(request, "dingdan"); // 执行删除命令
