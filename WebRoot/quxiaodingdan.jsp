...
if (a.equals("insert")) {
    // 执行插入取消订单模块,表名：quxiaodingdan
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().insert(request, "quxiaodingdan", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    new CommDAO().commOper("update dingdan set dingdanzhuangtai='已取消' where id='" + request.getParameter("dingdanid") + "'");
    ...
    // 执行更新取消订单模块,表名：quxiaodingdan
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().update(request, "quxiaodingdan", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    Map map = Query.make("quxiaodingdan").find(Request.get("id"));

    new CommDAO().delete(request, "quxiaodingdan"); // 执行删除命令
